...
static void encode_sb(const AV1_COMP *const cpi, ThreadData *td,
                      ...
                      PC_TREE *pc_tree, PARTITION_TREE *ptree,
...
void av1_rd_use_partition(AV1_COMP *cpi, ThreadData *td, TileDataEnc *tile_data,
  ...
  int splits_below = 0;
#if CONFIG_SDP
  BLOCK_SIZE bs_type = mib[0]->sb_type[plane_type];
#else
...
#if CONFIG_SDP
        if (this_mi && this_mi->sb_type[plane_type] >= sub_subsize) {
#else
...
#if CONFIG_SDP
      mib[0]->sb_type[plane_type] = bs_type;
#else
 ...
 * \param[in]    mi_row         Row coordinate of the block in a step size of
 *                              MI_SIZE
 * \param[in]    mi_col         Column coordinate of the block in a step size
 * of MI_SIZE \param[in]    rd_cost        Pointer to structure holding rate
 * and distortion stats for the current block \param[in]    bsize Current
 * block size \param[in]    ctx            Pointer to structure holding coding
 * contexts and chosen modes for the current block
 *
 * \return Nothing is returned. Instead, the chosen modes and contexts
 * necessary for reconstruction are stored in ctx, the rate-distortion stats
 * are stored in rd_cost. If no valid mode leading to rd_cost <= best_rd, the
 * status will be signalled by an INT64_MAX rd_cost->rdcost.
 */
...
#if CONFIG_SDP
        none_rdc.rate +=
            mode_costs->partition_cost[plane_type][pl][PARTITION_NONE];
#else
        none_rdc.rate += mode_costs->partition_cost[pl][PARTITION_NONE];
#endif
        none_rdc.rdcost = RDCOST(x->rdmult, none_rdc.rate, none_rdc.dist);
...
#if CONFIG_SDP
        split_rdc.rate +=
            mode_costs->partition_cost[plane_type][pl][PARTITION_SPLIT];
#else
...
#if CONFIG_SDP
          mib[0]->sb_type[plane_type] = subsize;
#else
  ...
  for (RECT_PART_TYPE i = HORZ; i < NUM_RECT_PARTS; i++) {
    ...
    if (i == HORZ) {
      ...
      const int track_ptree_luma =
          ptree_luma && ptree_luma->partition == PARTITION_HORZ;
#endif  // CONFIG_SDP
      ...
      av1_rd_pick_partition(cpi, td, tile_data, tp, blk_params.mi_row,
                            ...
                            best_remain_rdcost, pc_tree->horizontal[0],
#if CONFIG_SDP
                            track_ptree_luma ? ptree_luma->sub_tree[0] : NULL,
#endif  // CONFIG_SDP
      ...
      if (sum_rdc->rdcost < best_rdc->rdcost && blk_params.has_rows) {
        ...
        av1_rd_pick_partition(cpi, td, tile_data, tp,
                              blk_params.mi_row + blk_params.mi_step_h,
                              blk_params.mi_col, blk_params.subsize, &this_rdc,
                              best_remain_rdcost, pc_tree->horizontal[1],
#if CONFIG_SDP
                              track_ptree_luma ? ptree_luma->sub_tree[1] : NULL,
#endif  // CONFIG_SDP
                              NULL, NULL, multi_pass_mode, NULL);
...
#if CONFIG_SDP
      const int track_ptree_luma =
          ptree_luma && ptree_luma->partition == PARTITION_VERT;
#endif  // CONFIG_SDP
      av1_rd_stats_subtraction(x->rdmult, best_rdc, sum_rdc,
                               &best_remain_rdcost);
      av1_rd_pick_partition(cpi, td, tile_data, tp, blk_params.mi_row,
                            blk_params.mi_col, blk_params.subsize, &this_rdc,
                            best_remain_rdcost, pc_tree->vertical[0],
#if CONFIG_SDP
                            track_ptree_luma ? ptree_luma->sub_tree[0] : NULL,
#endif  // CONFIG_SDP
                            NULL, NULL, multi_pass_mode, NULL);
...
#if CONFIG_SDP
      ptree_luma ? ptree_luma->sub_tree[i] : NULL,
#endif  // CONFIG_SDP
      NULL,
      this_mi_row,
      mi_col,
...
bool av1_rd_pick_partition(AV1_COMP *const cpi, ThreadData *td,
  ...
  if (xd->tree_type == CHROMA_PART && min_bsize_1d >= SHARED_PART_SIZE &&
    ...
    // TODO(yuec): Need to make sure there is at least one valid partition mode
  }
#else   // CONFIG_EXT_RECUR_PARTITIONS
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
